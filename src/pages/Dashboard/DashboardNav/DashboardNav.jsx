  ...
  return (
    <nav className="h-16 bg-white border-b border-slate-100 px-4 flex items-center justify-between sticky top-0 z-30 shadow-sm shrink-0">
      ...
      <div className="hidden md:flex items-center gap-6">
        <Link to="/dashboard" className="text-sm font-bold text-slate-500 hover:text-emerald-600 transition-colors">
          Overview
        </Link>
        <Link to="/dashboard/my-plan" className="text-sm font-bold text-slate-500 hover:text-emerald-600 transition-colors">
          My Plan
        </Link>
        <Link to="/dashboard/settings" className="text-sm font-bold text-slate-500 hover:text-emerald-600 transition-colors">
          Settings
        </Link>
      </div>
      ...
      <span className="absolute top-2 right-2.5 w-2 h-2 bg-emerald-500 rounded-full border-2 border-white group-hover:animate-pulse"></span>
        ...
        <div className="relative">
          <button
            ...
            className={`flex items-center gap-2 p-1 pr-3 rounded-full border transition-all ${isUserMenuOpen ? "border-emerald-200 bg-emerald-50" : "border-slate-100 hover:bg-slate-50"
              }`}
          >
            {/* Dynamic Avatar Fix */}
